...
seed_val = 2023
# %% parameter settings
n_vec = np.array([25], dtype=np.int32)  # the length of time series
n_len = len(n_vec)
epsilon = 0.05
...
cusum_result_folder = Path(file_path.parents[2], "datasets")
print(cusum_result_folder)
current_file = "NN_train"

logdir_base = Path("tensorboard_logs")
logdir = Path(logdir_base, current_file + f"_rho_50")
...
for i in range(n_len):
    ...
    for j in range(num_N):
        ...
        data_null = GenDataMeanAR(N, n, cp=None, mu=(mu_L, mu_L), sigma=sigma, coef=rho)  # Use calculated N_no_change
...
pkl_filename = current_file + f"onlinetrain_50.pkl"
