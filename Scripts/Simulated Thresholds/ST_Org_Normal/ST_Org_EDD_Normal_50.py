...
model_name = "n50N400m20l5" #modell navn fra tensorboard, "vindu,antall serier trent på, bredde på NN, layers"
logdir = Path("tensorboard_logs", f"{current_file}_50") 
...
arl_logit_cusum = np.zeros((len(percentiles_logit),num_repeats))
...
for idx in range(len(percentiles_nn)): #looper gjennom alle percentilene
    ...
    for i in range(num_repeats):
        ...
        dt_logit_cusum, _ = detect_change_in_stream_batched_cusum(data[i], model, window_length, current_threshold_logit)
        if dt_logit_cusum > 0:
...
detection_delay_logit_cusum = np.zeros(len(percentiles_logit))
...
for p_idx in range(len(percentiles_nn)):
    ...
    for i in range(num_repeats):
        ...
        delay_logit, delay_count_logit, fps_logit, fns_logit = find_detection_delay(stream_length, true_tau_alt[i], dt_logit_cusum)
        ...
        # logit cusum
        detection_delay_logit_cusum[p_idx] += delay_logit
    ...
    detection_delay_logit_cusum[p_idx] /= detections_count_logit[p_idx]
...
plt.plot(false_alarm_rates, detection_delay_smart_cusum, 'o-', linewidth=1.5, markersize=3, color='purple') #smart cusum er lilla
plt.xlim(0.8,1.0)
...
plt.grid(True)
plt.legend(['NN', 'CUSUM', 'Logit', 'Smart CUSUM'])
plt.savefig(f"Figures/ST_Org_EDD_Normal_50_50000bigrun.png")
plt.tight_layout()
#plt.show() 
