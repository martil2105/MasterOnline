...
for p_idx in range(len(percentiles_nn)):
    ...
    for i in range(num_repeats):
        ...
        if dt_nn == 0 and true_tau_alt[i] > 0:
            fn_nn[p_idx] += 1
            detection_times_nn[p_idx] += stream_length
            dt_nn_count += 1
        ...
        if dt_cusum == 0 and true_tau_alt[i] > 0:
            fn_cusum[p_idx] += 1
            detection_times_cusum[p_idx] += stream_length
            dt_cusum_count += 1
        ...
        if dt_logit_cusum == 0 and true_tau_alt[i] > 0:
            ...
            detection_times_logit_cusum[p_idx] += stream_length
            dt_logit_cusum_count += 1
...
plt.savefig(f"Figures/ST_SL_EDD_Normal.png")
plt.tight_layout()
#plt.show()