...
for idx, percentile in enumerate(percentiles):
    for i in range(num_repeats):
        ...
        if dt > 0 and dt < true_tau_alt[i]:
            fp[idx] += 1
        #if dt > 0 and dt-true_tau_alt[i] > window_length:
        #    fp[idx] += 1
...
for idx, percentile in enumerate(percentiles_cusum):
    for i in range(num_repeats):
        ...
        #if dt_cusum > 0 and dt_cusum-true_tau_alt[i] > window_length:
        #    fp_cusum[idx] += 1
...
for idx, percentile in enumerate(percentiles_logit_cusum):
    for i in range(num_repeats):
        ...
        #if dt_logit_cusum > 0 and dt_logit_cusum-true_tau_alt[i] > window_length:
        #    fp_logit_cusum[idx] += 1
...
plt.savefig(f"Figures/ST_Org_EDD_Cauchy.png")
plt.tight_layout()
#plt.show()
