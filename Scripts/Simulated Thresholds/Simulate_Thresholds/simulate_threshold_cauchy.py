...
for i in range(num_streams):
    ...
    dt_logit, max_prob_logit = detect_change_in_stream_batched_cusum(data_null[i], model, window_length, thresholds[0])
    max_probabilities_logit.append(max_prob_logit)
    dt_smart_cusum, max_prob_smart_cusum = smart_li_cusum(data_null[i], window_length, thresholds[0])
    max_probabilities_smart_cusum.append(max_prob_smart_cusum)
