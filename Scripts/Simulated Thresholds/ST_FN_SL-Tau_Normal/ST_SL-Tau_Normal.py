...
detection_times_nn = np.zeros(len(percentiles_nn))
detection_times_cusum = np.zeros(len(percentiles_cusum))
detection_times_logit_cusum = np.zeros(len(percentiles_logit_cusum))
...
average_nn_delay = np.zeros(len(percentiles_nn))
average_cusum_delay = np.zeros(len(percentiles_cusum))
average_logit_cusum_delay = np.zeros(len(percentiles_logit_cusum))
for p_idx in range(len(percentiles_nn)):
    ...
    for i in range(num_repeats):
        ...
        if dt_nn > 0 and dt_nn >= true_tau_alt[i]:
            detection_times_nn[p_idx] += dt_nn-true_tau_alt[i]
            dt_nn_count += 1
        if dt_nn > 0 and dt_nn < true_tau_alt[i]:
            fp_nn[p_idx] += 1
        if dt_nn == 0 and true_tau_alt[i] > 0:
            fn_nn[p_idx] += 1
            detection_times_nn[p_idx] += stream_length-true_tau_alt[i]
            dt_nn_count += 1
        dt_cusum, _ = li_cusum(data_alt[i], window_length, current_threshold_cusum)
        if dt_cusum > 0 and dt_cusum >= true_tau_alt[i]:
            detection_times_cusum[p_idx] += dt_cusum-true_tau_alt[i]
            dt_cusum_count += 1
        if dt_cusum > 0 and dt_cusum < true_tau_alt[i]:
            fp_cusum[p_idx] += 1
        if dt_cusum == 0 and true_tau_alt[i] > 0:
            fn_cusum[p_idx] += 1
            detection_times_cusum[p_idx] += stream_length-true_tau_alt[i]
            dt_cusum_count += 1
        ...
        if dt_logit_cusum > 0 and dt_logit_cusum >= true_tau_alt[i]:
            detection_times_logit_cusum[p_idx] += dt_logit_cusum-true_tau_alt[i]
            dt_logit_cusum_count += 1
        if dt_logit_cusum > 0 and dt_logit_cusum < true_tau_alt[i]:
            fp_logit_cusum[p_idx] += 1
        if dt_logit_cusum == 0 and true_tau_alt[i] > 0:
            fn_logit_cusum[p_idx] += 1
            detection_times_logit_cusum[p_idx] += stream_length-true_tau_alt[i]
            dt_logit_cusum_count += 1
    average_nn_delay[p_idx] += detection_times_nn[p_idx]/dt_nn_count
    average_cusum_delay[p_idx] += detection_times_cusum[p_idx]/dt_cusum_count
    average_logit_cusum_delay[p_idx] += detection_times_logit_cusum[p_idx]/dt_logit_cusum_count
